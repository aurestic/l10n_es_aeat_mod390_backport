...
class L10nEsAeatMod390Report(models.Model):
    ...
    def __init__(self, pool, cr):
        self._aeat_number = '390'
        super(L10nEsAeatMod390Report, self).__init__(pool, cr)

    number = fields.Char(default='390')
    ...
    other_first_activity_code = fields.Selection(
        ...
        string=u"Código 1ª actividad (antiguo)", readonly=True,
    )
    ...
    other_second_activity_code = fields.Selection(
        selection=ACTIVITY_CODE_SELECTION, states=EDITABLE_ON_CALCULATED,
        string=u"Código 2ª actividad (antiguo)", readonly=True,
    )
    ...
    other_third_activity_code = fields.Selection(
        selection=ACTIVITY_CODE_SELECTION, states=EDITABLE_ON_CALCULATED,
        string=u"Código 3ª actividad (antiguo)", readonly=True,
    )
    ...
    other_fourth_activity_code = fields.Selection(
        selection=ACTIVITY_CODE_SELECTION, states=EDITABLE_ON_CALCULATED,
        string=u"Código 4ª actividad (antiguo)", readonly=True,
    )
    ...
    other_fifth_activity_code = fields.Selection(
        selection=ACTIVITY_CODE_SELECTION, states=EDITABLE_ON_CALCULATED,
        string=u"Código 5ª actividad (antiguo)", readonly=True,
    )
    ...
    def _compute_casilla_33(self):
        for report in self:
            report.casilla_33 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
                    1, 3, 702, 5,  # Régimen ordinario
    ...
    def _compute_casilla_34(self):
        for report in self:
            report.casilla_34 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
                    2, 4, 703, 6,  # Régimen ordinario
    ...
    @api.multi
    @api.depends('casilla_34', 'tax_lines')
    ...
    def _compute_casilla_38(self):
        for report in self:
            report.casilla_38 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
    ...
    def _compute_casilla_39(self):
        for report in self:
            report.casilla_39 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
    ...
    def _compute_casilla_48(self):
        for report in self:
            report.casilla_48 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
    ...
    def _compute_casilla_49(self):
        for report in self:
            report.casilla_49 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
    ...
    def _compute_casilla_53(self):
        for report in self:
            report.casilla_53 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
    ...
    def _compute_casilla_56(self):
        for report in self:
            report.casilla_56 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
    ...
    def _compute_casilla_57(self):
        for report in self:
            report.casilla_57 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
    ...
    def _compute_casilla_598(self):
        for report in self:
            report.casilla_598 = sum(report.tax_lines.filtered(
                lambda x: x.field_number in (
